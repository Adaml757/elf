function meandata = elf_analysis_datasetmean(data, sel, datasetMeanType)
...
%
%   Example:
%   meandata = elf_analysis_datasetmean(data, sel, para.plot.datasetMeanType)  
%
% Inputs: 
...
%% Check inputs
if nargin < 3 || isempty(datasetMeanType), datasetMeanType = 'logmean'; end
if nargin < 2 || isempty(sel), sel = 1:length(data); end
    
%% initialise from first element to get all label fields and histbins
meandata = data(1); %% Note: This might be a problem if the data structure ever varies across exposures
